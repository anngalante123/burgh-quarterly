...
## 2026-06-12 — Audit verification + Batch 1 data-quality fixes (shipped)

Fact-checked the 9-point P0/P1/P2 audit against Neon + content/social. Verdicts: #1 "ratings only for 30" FALSE (2,478/2,580 had google_rating; real gaps are review keywords (17 slugs) and review depth (max 20 stored/business)). #2 posts_30d clip at 12 TRUE. #3 engagement unbounded TRUE (1,739% on 653 followers). #4 chains TRUE. #5 "dupes" MOSTLY multi-location locals (legit) + 29 WRONG IG attributions (squarespace x7, wix x5, culturaltrust, omnihotels, etc.) + exactly 1 true dup. #6 sentiment sample TRUE (~10.6 avg reviews). #7 geo gaps overstated (all rows had address+category; 1,184 had generic neighborhood). #8 freshness split TRUE. #9 dirty names live in IG full_name (62), DB names clean; Jeni's/Everyday Noodles NOT miscategorized.

Shipped (all DB writes snapshotted in scripts/backups/):
- Chain blocklist +5 (raising cane, the melting pot, city works, the fresh market, ross dress for less) + their 5 DB rows deleted (scripts/remove-audit-chains.ts).
- Engagement sanitizer lib/data/engagement.ts (100-follower floor, 30% credibility ceiling, returns 0) wired into loadSocialBySlug, momentumScore, generate-engagement-rate-posts. 6 tests.
- Geo backfill (scripts/backfill-geo.ts): 1,553 lat/lng from raw Apify (DB previously had ZERO coords), 671 neighborhoods (667 suburb-parse + 4 polygon via data/pgh-neighborhoods.geojson, WPRDC, 90 hoods). Generic "Pittsburgh" rows: 1,184 -> 512. KEY INSIGHT: most remaining 512 are USPS-Pittsburgh SUBURBS outside city polygons; need Allegheny County municipal boundaries GeoJSON (WPRDC) next.
- Wrong-IG fix (scripts/fix-wrong-ig-attributions.ts): 29 social JSONs nulled (error: wrong_attribution, growth/tiktok preserved), handles.json marked rejected_wrong_attribution to block re-match. Deleted dup burghers-brewing-tap-house-lawrenceville (49 reviews; kept -company- listing, 701 reviews).
