...
export function YourClimb({ framing, stats }: Props) {
  return (
    <Reveal as="section" className="block">
      <div className="bg-brand-purple text-brand-lavender px-5 py-6 md:px-8 md:py-8 rounded-lg">
        <div className="grid grid-cols-1 md:grid-cols-[auto_1fr_1fr_1fr] gap-5 md:gap-8 items-center">
          <div>
            <p className="font-display text-[0.58rem] font-semibold uppercase tracking-[0.22em] text-brand-lavender/65">
              {framing}
            </p>
            <p className="mt-1 font-display font-black uppercase tracking-[-0.02em] text-brand-lavender text-3xl md:text-4xl leading-[0.95]">
              Your{" "}
          ...
          {stats.map((s, i) => {
            ...
            const arrowColor =
              dir === "up"
                ...
                : dir === "down"
                  ? "text-brand-cream"
                  : "text-brand-lavender/55";
            return (
              <div
                key={i}
                className="border-l-0 md:border-l border-brand-lavender/20 md:pl-6 pt-3 md:pt-0 first-of-type:border-0 first-of-type:pl-0 first-of-type:pt-0"
              >
                <p className="font-display text-[0.58rem] font-semibold uppercase tracking-[0.22em] text-brand-lavender/65">
                  {s.label}
                </p>
                <div className="mt-1 flex items-baseline gap-2">
                  <span className="font-display text-2xl md:text-3xl font-black tabular-nums leading-none text-brand-lavender">
                    {s.value}
                  </span>
                  <span
                    aria-hidden="true"
                ...
                <p className="mt-1.5 font-body text-[0.72rem] text-brand-lavender/65">
                  {s.sub}
                </p>
              </div>
            );
